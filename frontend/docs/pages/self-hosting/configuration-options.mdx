...
| Variable                                            | Description                                                                                                | Default Value    |
| --------------------------------------------------- | ---------------------------------------------------------------------------------------------------------- | ---------------- |
| `SERVER_MSGQUEUE_KIND`                              | Message queue kind (`rabbitmq`, `postgres`, or `nats`)                                                     | `rabbitmq`       |
...
| `SERVER_MSGQUEUE_NATS_URL`                          | NATS seed URL(s), comma-separated for a cluster (requires JetStream)                                       |                  |
| `SERVER_MSGQUEUE_NATS_USERNAME`                     | NATS username (sent as a connect option, so reconnects also authenticate)                                  |                  |
| `SERVER_MSGQUEUE_NATS_PASSWORD`                     | NATS password (see username)                                                                               |                  |
| `SERVER_MSGQUEUE_NATS_QOS`                          | NATS QoS, applied as the consumer's `MaxAckPending`                                                        | `100`            |
| `SERVER_MSGQUEUE_NATS_COMPRESSION_ENABLED`          | Enable gzip compression of messages                                                                        | `false`          |
| `SERVER_MSGQUEUE_NATS_COMPRESSION_THRESHOLD`        | Byte size above which messages are compressed                                                              | `5120`           |
| `SERVER_MSGQUEUE_NATS_ENABLE_MESSAGE_REJECTION`     | Stop redelivering messages past the max death count                                                        | `false`          |
| `SERVER_MSGQUEUE_NATS_MAX_DEATH_COUNT`              | Max redeliveries before a message is rejected                                                              | `1000`           |
| `SERVER_MSGQUEUE_NATS_ASYNC_PUBLISH`                | Pipeline publishes instead of awaiting a server ack per message (see below)                                | `false`          |
| `SERVER_MSGQUEUE_NATS_TOKEN`                        | NATS token auth                                                                                            |                  |
| `SERVER_MSGQUEUE_NATS_CREDENTIALS_FILE`             | Path to a NATS `.creds` file (user JWT + seed)                                                             |                  |
| `SERVER_MSGQUEUE_NATS_NKEY_SEED_FILE`               | Path to an NKey seed file                                                                                  |                  |
| `SERVER_MSGQUEUE_NATS_TLS_STRATEGY`                 | Transport security: `none`, `tls`, or `mtls`                                                               | `none`           |
| `SERVER_MSGQUEUE_NATS_TLS_ROOT_CA_FILE`             | CA bundle used to verify the NATS server (needed for a private CA)                                         |                  |
| `SERVER_MSGQUEUE_NATS_TLS_CERT_FILE`                | Client certificate for mTLS                                                                                |                  |
| `SERVER_MSGQUEUE_NATS_TLS_KEY_FILE`                 | Client key for mTLS                                                                                        |                  |
| `SERVER_MSGQUEUE_NATS_TLS_SERVER_NAME`              | Overrides the name verified against the server certificate                                                 |                  |
| `SERVER_MSGQUEUE_NATS_TLS_MIN_VERSION`              | Minimum TLS version, `1.2` or `1.3`                                                                        | `1.3`            |
...
NATS can back either half of Hatchet's messaging, and the two halves are
configured independently:

- **Durable task queues** (`SERVER_MSGQUEUE_KIND=nats`) run on JetStream, which
  must be enabled on the server. Delivery is at-least-once with explicit acks.
- **Best-effort pub/sub** (`SERVER_MSGQUEUE_PUBSUB_KIND=nats`) uses core NATS,
  no JetStream required. Delivery is at-most-once.

Setting `SERVER_MSGQUEUE_KIND=nats` makes the pub/sub default to NATS too, and
it inherits the durable queue's URL and credentials, so a NATS-only deployment
needs just:
...
The two halves use different subject namespaces (`hatchet.mq` and
`hatchet.pubsub`) and always open separate connections, so one NATS server can
carry both.

#### Authentication and transport security

Both halves accept the same credentials, and the pub/sub inherits the durable
queue's when it has none of its own — so a NATS-only deployment configures them
once.

Exactly one authentication mechanism may be set. Configuring two is rejected at
startup rather than resolved by precedence, because silently ignoring
credentials an operator deliberately set is how a connection ends up
authenticating as something other than intended.

| Mechanism | Variables |
| --- | --- |
| User and password | `..._NATS_USERNAME`, `..._NATS_PASSWORD` |
| Token | `..._NATS_TOKEN` |
| User JWT (operator accounts, Synadia Cloud) | `..._NATS_CREDENTIALS_FILE` |
| NKey | `..._NATS_NKEY_SEED_FILE` |

Transport security is off by default, matching a stock NATS server, and is
turned on explicitly:

```sh
SERVER_MSGQUEUE_NATS_TLS_STRATEGY=tls
SERVER_MSGQUEUE_NATS_TLS_ROOT_CA_FILE=/etc/hatchet/nats-ca.pem
```

The root CA is only needed when the server presents a certificate from a
private CA; a publicly-signed certificate verifies against the system trust
store. For mutual TLS, set the strategy to `mtls` and supply a client
certificate and key — Hatchet refuses to start if `mtls` is set without them.

Setting a strategy other than `none` makes TLS mandatory for that connection:
it fails rather than falling back to plaintext, so a misconfiguration surfaces
as a failed startup instead of an unencrypted link. There is deliberately no
option to skip certificate verification.

Each variable has a `SERVER_MSGQUEUE_PUBSUB_NATS_*` counterpart for configuring
the pub/sub separately, which is what you want when the two connect to
different servers or authenticate as different users.
...
For the durable queue, JetStream must be enabled and sized for your task
backlog. Hatchet creates one work-queue-retention stream per queue, so a stream
holds every message that has not yet been acked; a stalled consumer is bounded
by `max_file_store` rather than by memory.

Prefer bare hosts in the URL and set the username and password separately.
Credentials embedded in the URL are not reapplied when the client reconnects to
a cluster peer it learned about through gossip.

#### Sharing a NATS server

Subjects and JetStream stream names are fixed (`hatchet.mq`, `hatchet.pubsub`,
and `HATCHET_*` streams). Two Hatchet installations therefore cannot share a
NATS account: they would publish to the same subjects and contend for the same
streams.

Give each installation its own NATS account — the isolation boundary NATS
itself provides — or its own server.

#### Publish throughput

By default every publish waits for JetStream to acknowledge it, so a successful
send means the message is persisted. That costs one network round trip per
message — measured at ~250µs against a loopback server, and proportionally more
against a real network — and the engine publishes from several serial loops
(task assignment, event ingest) that were written against RabbitMQ, where a
publish is an unconfirmed socket write.

Setting `SERVER_MSGQUEUE_NATS_ASYNC_PUBLISH=true` pipelines those publishes
instead, which measured ~19x on the publish path (3.6k to 70k messages/second
for a single serial publisher on loopback). Publishes remain bounded so a broker
that stops acknowledging applies backpressure rather than growing the engine's
memory without limit.

The trade is what a successful send means: with async publishing it means the
client has accepted the message, not that the server has stored it. A rejection
that arrives afterwards is logged and marks the queue unhealthy, but the code
that published has already moved on. This is the same guarantee the RabbitMQ
backend gives — it publishes without confirms — so the engine is built for it.
It is nonetheless weaker than this backend's default, which is why it is opt-in.

Measure before enabling it, because publishing is usually not what limits task
throughput. On a single-node setup with NATS on loopback, turning it on moved
end-to-end task throughput from 325 to 347 tasks/second across three runs each —
inside the run-to-run spread, despite the 19x on the publish path itself. The
queue simply was not the bottleneck: that deployment asked about 1,000
messages/second of a backend that drains 50,000.

It is worth enabling when publishing is genuinely the constraint. The clearest
case is a NATS server that is a network hop away rather than a sidecar, since
synchronous publishing costs one round trip per message and therefore degrades
in direct proportion to the round-trip time, while pipelined publishing does
not.

#### How async publishing compares with RabbitMQ

The comparison runs both ways, and it is worth being precise because "we might
lose a message" sounds worse than it is.

RabbitMQ publishes **without confirms**. A publish there returns once the frames
are written to the socket, and nothing ever confirms that the broker accepted,
routed or persisted them — a dropped message is reported nowhere. Async NATS
still receives a JetStream acknowledgement for every publish; a rejection
reaches the error handler, is logged, and marks the queue unhealthy so it stops
reporting itself as ready. On that dimension async NATS is the stronger of the
two.

What is weaker is narrower. The NATS client appends to a 32 KiB write buffer and
wakes a flusher goroutine, where the AMQP client flushes to the socket inline,
so a hard kill (`SIGKILL`, OOM) can lose what has not flushed yet — a window of
microseconds. A graceful shutdown does not: the client flushes its outbound
buffer before closing.

Neither is as strong as the synchronous default, which returns only once
JetStream has persisted and replicated the message.

The reason any of this is tolerable is that **the scheduler's queue lives in
Postgres**, not in the broker. Task messages wake a loop that also polls on its
own, so a lost message delays work rather than losing it. That is why the
synchronous default is the right one for correctness, and why async is a
reasonable trade when publishing is genuinely the constraint.

Batch publishing (NATS 2.12+ atomic batches, 2.14+ fast ingest) is deliberately
not used. Published figures put async publishing at or above atomic batching, so
the batching itself buys nothing here; the large numbers quoted for fast ingest
come from pairing it with a stream in `persist_mode: async`, which is a separate
setting that trades away more durability. Hatchet also sends one message per
call, so there is no batch to form without duplicating the buffering the engine
already does above this layer.

#### Behavioral differences from RabbitMQ

The durable queue matches the RabbitMQ backend's observable behavior, but
reaches it through JetStream primitives rather than AMQP ones:

- **Retries.** RabbitMQ parks a failed message in a pair of ping-ponging
  dead-letter queues to apply the backoff. JetStream expresses this directly, so
  a failed message is redelivered on the same consumer after the backoff and
  there are no automatic dead-letter queues to observe.
- **Publishes are confirmed.** JetStream acknowledges every publish, so a
  successful `SendMessage` means the message is persisted. The RabbitMQ backend
  publishes without confirms.
- **Dispatcher message expiry.** RabbitMQ has the broker enforce the dispatcher
  queue's 20-second message TTL via a dead-letter exchange. JetStream has no
  equivalent, so the deadline is enforced when the message is delivered: an
  expired message is routed to the dispatcher dead-letter queue instead of being
  handled. A message that no consumer ever receives — because the dispatcher is
  gone entirely — is dropped when the stream ages it out, and the affected tasks
  are recovered by the engine's reassignment and timeout paths rather than
  through the dead-letter queue. This matches the Postgres backend, which also
