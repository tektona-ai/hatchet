...
Enable it if you are publish-bound, particularly when NATS is a network hop away
rather than a sidecar. Note that publish throughput is not always what limits
end-to-end task throughput; measure before enabling.

Batch publishing (NATS 2.12+ atomic batches, 2.14+ fast ingest) is deliberately
not used. Published figures put async publishing at or above atomic batching,
and Hatchet sends one message per call, so there is no batch to form without
duplicating the buffering the engine already does above this layer.
