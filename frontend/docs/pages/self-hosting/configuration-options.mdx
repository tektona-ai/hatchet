...
| `SERVER_MSGQUEUE_NATS_SUBJECT_PREFIX`               | NATS subject prefix for durable queues, joined to queue names with `.`                                     | `hatchet.mq`     |
| `SERVER_MSGQUEUE_NATS_STREAM_PREFIX`                | Prefix for the JetStream streams Hatchet creates                                                           | `HATCHET`        |
...
`SERVER_MSGQUEUE_PUBSUB_NATS_SUBJECT_PREFIX` and
`SERVER_MSGQUEUE_NATS_SUBJECT_PREFIX` to keep installations that share a NATS
server from seeing each other's messages.

Streams are a separate, flat namespace per NATS account, and the subject prefix
does not keep them apart. Two installations sharing an account must also set
different `SERVER_MSGQUEUE_NATS_STREAM_PREFIX` values, or they will collide on
stream names.
